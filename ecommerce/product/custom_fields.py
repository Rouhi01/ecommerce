...
class OrderField(models.PositiveIntegerField):
    ...
    def check(self, **kwargs):
        return [
            *super().check(**kwargs),
            *self.__check_for_field_attribute(**kwargs)
        ]

    def __check_for_field_attribute(self, **kwargs):
    ...
    def pre_save(self, model_instance, add):
        if getattr(model_instance, self.attname) is None:
            product_lines = self.model.objects.all()
            try:
                query = {self.unique_for_field: getattr(
                    model_instance, self.unique_for_field
                )}
                qs = product_lines.filter(**query)
